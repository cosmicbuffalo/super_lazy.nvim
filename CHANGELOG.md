...
### Added
- **`silent` config option**: New boolean option (default: `false`) to suppress automatic notifications
  - When `true`, background operations like startup sync and lazy update hooks run silently (no fidget progress indicators)
  - User-triggered commands (`:SuperLazyRefresh`, `:SuperLazyDebug`) still show full feedback
